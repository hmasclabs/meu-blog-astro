...
export default function PostEditor({ filePath }: PostEditorProps) {
    ...
    const handleTitleChange = (val: string) => {
        setPost(p => ({ ...p, title: val, slug: isEditing ? p.slug : val.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') }));
    };
                ...
                <div className="flex-1 min-w-0 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="mt-3">
                            ...
                            <input type="text" value={post.slug} onChange={e => setPost(p => ({ ...p, slug: e.target.value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9-]/g, '-') }))} className={`${inputClass} font-mono text-xs`} placeholder="url-do-artigo" />
                        </div>
                        <div className="mt-3">
